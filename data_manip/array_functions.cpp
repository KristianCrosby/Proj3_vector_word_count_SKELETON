...
namespace KP{
	...
	void clear(vector<constants::entry>  &entries){
		entries.clear();
	}
	...
	//get word at location i if i<size(see above function), otherwise return the last word
	//(this is lazy, should throw an exception instead)
	string getWordAt(vector<constants::entry>  &entries, int i){
		return entries.at(i).word;
	}

	//get number_occurences at location i if <size(see above function), otherwise return the last number_occurences
	//(this is lazy, should throw an exception instead)
	int getNumbOccurAt(vector<constants::entry>  &entries,int i){
		if (entries.size() == 0){
			return FAIL;
		}
	...
	*         true: otherwise*/
	bool processFile(vector<constants::entry>  &entries, fstream &myfstream){
		if(!myfstream){
			return false;
		}
		string line1;
	...
	*/
	...
	void sort(vector<constants::entry>  &entries, constants::sortOrder so){
				if(so == NONE) {
					return;
				}
				if (so == ASCENDING) {
